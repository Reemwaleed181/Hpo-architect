...
export default function HPOReasoningPath({ exp, analysis }:{ exp: Experiment, analysis?: AnalysisResult }){
  ...
  return (
    <div className="card mt-4">
      ...
      <div className={`mt-4 transition-all ${open? 'max-h-[3000px]':'max-h-0 overflow-hidden'}`}>
        ...
        {trace.length===0 ? (
          ...
          <div key={i} className="border-t border-slate-800 py-4">
            <div className="grid grid-cols-1 gap-2 md:grid-cols-[minmax(0,1fr)_2rem_minmax(0,1.2fr)_2rem_minmax(0,0.9fr)] md:items-center md:gap-2">
            ...
            <div className="px-3"><ScientificEvidence referenceIds={t.referenceIds} compact /></div>
          </div>
        ))}
        ...
        <div className="mt-2 border-l-4 border-cyan-400 bg-slate-900 px-4 py-3">
          ...
          <div className="mt-2 text-xs text-slate-400">{analysisLocal.scientificDisclosure}</div>
        </div>
      </div>
    </div>
  )
